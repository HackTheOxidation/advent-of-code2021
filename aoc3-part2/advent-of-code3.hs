...
filterByBitPosition :: [Int] -> [[Int]] -> [[Int]]
filterByBitPosition [] bss = bss
filterByBitPosition (g : gs) bss = filterByBitPosition gs (filter (\bs -> g == reverse bs !! length gs) bss)

filterBinary :: [Int] -> [[Int]] -> [Int]
filterBinary common binaryNumbers = head (filterByBitPosition common binaryNumbers)

findRating :: [Int] -> [[Int]] -> Int
findRating common binaryNumbers = binToDec (filterBinary common binaryNumbers)
...
main = do
  ...
  let oxygenGeneratorRating = findRating gamma binaryNumbers
  print oxygenGeneratorRating
  let co2rating = findRating epsilon binaryNumbers
  print co2rating
